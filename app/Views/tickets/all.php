...
<ul class="nav nav-pills ticket-list-tabs mb-4">
    <li class="nav-item">
        <a class="nav-link active" href="/tickets/all">Tous les tickets</a>
    </li>
    <li class="nav-item">
                    ...
                    <tr>
                        ...
                        <td class="text-end">
                            <?php if (($currentRole ?? null) === 'TECHNICIAN' && $ticket['statut'] === 'NOUVEAU'): ?>
                                <form method="post" action="/tickets/<?= e((string) $ticket['id']) ?>/assign" class="d-inline">
                                    <?= csrf_field() ?>
                                    <button type="submit" class="btn btn-sm btn-outline-dark">Prendre en charge</button>
                                </form>
                            <?php else: ?>
